    ...
    <div className={style.h}>
      <Container className='px-5'>
        <h2
          ...
          className={clsx(styles.cardTitle, 'py-5')}
          onClick={goBack}
        >
          {' '}
          <span className='border'>{`<`}</span> Gig details
        </h2>
        <Card style={{ boxShadow: '0 0 5px rgba(0, 0, 0, 0.3)' }} className={clsx(styles.card)}>
          ...
          <div style={{paddingLeft:"1000px"}}>
            ...
            <div>
              <h3 className={styles.cardTitle}>Title of the GIG : {state.title}</h3>
              <Card.Text className={styles.cardText}>
                <h3>Discription</h3>
                {state.description}
              </Card.Text>
              <div className={styles.prices}>
                <h3 className={styles.cardTitle}>GIG Price : {state.price}</h3>
              </div>
              <img src={state.imageUrl || gigImage} alt='' className='rounded-4' />
            </div>
          </Card.Body>
        </Card>
...
const PaymentModal = ({ open, setOpen }) => {
  ...
  return (
    <Modal show={open} onHide={handleClose}>
      ...
      <Modal.Body>
        <Payment handleClose={handleClose} toast={toast} />
      </Modal.Body>
    </Modal>
  );
};
